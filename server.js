...
//  2. POST /api/proxy  — proxies requests to api.createsend.com (avoids browser CORS)
//  3. POST /api/smtp   — sends via CM SMTP relay (server-side, no port blocks)
...
// Proxies to https://api.createsend.com — bypasses browser CORS restriction.
...
// Tries ports 2525 → 465 → 587 in order.
...
app.post('/api/smtp', async (req, res) => {
  ...
  for (const portConfig of SMTP_PORTS) {
    try {
      ...
      // Auth failure — no point trying other ports
...
app.listen(PORT, () => {
  console.log(`CM Transactional Tester running on http://localhost:${PORT}`);
});
